...
/**
 * Single particle description: coordinate, momentum, energy, mass, collision
 * flags
 */
...
/**
 * This function takes in Particle pointer (*particle),
 * three component particle velocity and time step dt.
 * Change particle coordinate with: x_i = x_i + v_i * dt
 */
void moveParticleLinear(Particle *particle, double Vx, double Vy, double Vz,
                        double dt) {
  // x = x + v*dt
  particle->x = fma(Vx, dt, particle->x);
  particle->y = fma(Vy, dt, particle->y);
  particle->z = fma(Vz, dt, particle->z);
}

/**
 * Given bubble radius, speed, single particle info and time step dt
 * calculate time if/when point particle collides with a sphere
 */
double calculateTimeToWall(Particle particle, Bubble bubble, double t_dt) {
  double time1, time2;

  // p·p/E^2 - V_b^2 = v·v/E^2 - V_b^2 : p and v are 3-vectors
  double a =
      fma(particle.pX, particle.pX,
          fma(particle.pY, particle.pY, fma(particle.pZ, particle.pZ, 0.))) /
          pow(particle.E, 2) -
      bubble.speed * bubble.speed;

  // p·x/E - R_b * V_b = v·x/E - R_b * V_b : p and v are 3-vectors
  double b = fma(particle.x, particle.pX / particle.E,
                 fma(particle.y, particle.pY / particle.E,
                     fma(particle.z, particle.pZ / particle.E,
                         -bubble.radius * bubble.speed)));

  // x·x - R_b^2 : x is 3-vector
  double c =
      fma(particle.x, particle.x,
          fma(particle.y, particle.y,
              fma(particle.z, particle.z, -bubble.radius * bubble.radius)));

  // Calculate element under sqrt. If it is negative -> no real solution
  ...
  // Select result which is smallest positive value
  time1 = (-b - sqrt(d)) / a;
  time2 = (-b + sqrt(d)) / a;
...
/**
 * Given Particle, Bubble and radius where two objects collide,
 * calculate bubble normal such that normal
 * is directed towards false vacuum (deteremined if mass in >/< mass_out)
 */
void calculateNormal(double *n_x, double *n_y, double *n_z, Particle particle,
                     Bubble bubble, double collide_radius, double mass_in,
                     double mass_out) {
  // If M_in > M_out then normal is towards center of bubble
  if (mass_in > mass_out) {
    *n_x = -particle.x * bubble.gamma / collide_radius;
    *n_y = -particle.y * bubble.gamma / collide_radius;
    *n_z = -particle.z * bubble.gamma / collide_radius;
  }
...
/**
 * Given Particle calculate it's radius
 */
double calculateRadius(Particle particle) {
  return fma(particle.x, particle.x,
             fma(particle.y, particle.y, particle.z * particle.z));
}

/**
 * Given Particle and mass inside vacuum calculate particle's energy
 */
...
// Particle step kernel when assuming eta --> oo --> All particles stay inside
// the bubble
__kernel void particles_with_false_bubble_step_reflect(
  ...
  // after time dt
  double X_dt_x = fma(V_1, dt, particle.x);
  double X_dt_y = fma(V_2, dt, particle.y);
  double X_dt_z = fma(V_3, dt, particle.z);
  ...
  if (((X2 < bubble.radius2) && (M_in < M_out))) {
    ...
    // reflect particles that would to get out
      ...
      moveParticleLinear(&particle, V_1, V_2, V_3, timeToWall);
      ...
      moveParticleLinear(&particle, V_1, V_2, V_3, dt - timeToWall);
    }
  } else {
    particle.x = 100000;
    particle.y = 100000;
...
/**
 * Kernel for simulating particle collision with bubble wall in 3D
 */
__kernel void particles_with_bubble_step(
    __global Particle *t_particles, __global double *t_dP,
    __global char *t_interactedFalse, __global char *t_passedFalse,
    __global char *t_interactedTrue, __constant Bubble *t_bubble,
    __constant double *t_dt, __constant double *t_m_in,
  ...
  if (((X2 < bubble.radius2) && (M_in < M_out)) ||
      ((X2 > bubble.radius2) && (M_in > M_out))) {
    // If M_in < M_out -> Check if particle stays in
    // If M_in > M_out -> Check if particle stays out
    if (((X_dt2 < bubble.radiusAfterStep2) && (M_in < M_out)) ||
        ((X_dt2 > bubble.radiusAfterStep2) && (M_in > M_out))) {
      // X_1 = fma(V_1, dt, X_1);
      // X_2 = fma(V_2, dt, X_2);
      // X_3 = fma(V_3, dt, X_3);
      moveParticleLinear(&particle, V_1, V_2, V_3, dt);
      t_dP[gid] = 0.;
    ...
    // Maybe get outside
      ...
      collide_radius = bubble.radius + bubble.speed * timeToWall;
      ...
      // Particle bounces from the wall and stays where it was -> Stays in lower
      // mass region Particle keeps lower mass
      if (np * np < Delta_M2) {
        // P_i = P_i + 2 * np * n_i
        particle.pX = fma(np * 2., n_x, particle.pX);
        particle.pY = fma(np * 2., n_y, particle.pY);
      ...
      // Particle gets higher mass
        ...
        particle.pX = fma(np * (1. - sqrt(1. - Delta_M2 / pow(np, 2.))), n_x,
                          particle.pX);
        particle.pY = fma(np * (1. - sqrt(1. - Delta_M2 / pow(np, 2.))), n_y,
                          particle.pY);
        particle.pZ = fma(np * (1. - sqrt(1. - Delta_M2 / pow(np, 2.))), n_z,
                          particle.pZ);
        t_dP[gid] =
  ...
  // If M_in > M_out -> Then particle starts inside
    ...
    // If M_in > M_out -> Particle goes outside from the bubble to inside
      ...
      particle.pY =
          fma(np * (1 - sqrt(1 + Delta_M2 / pow(np, 2))), n_y, particle.pY);
      particle.pZ =
          fma(np * (1 - sqrt(1 + Delta_M2 / pow(np, 2))), n_z, particle.pZ);
...
__kernel void particles_with_bubble_step_cyclic(
    __global Particle *t_particles, __global double *t_dP,
    __global char *t_interactedFalse, __global char *t_passedFalse,
    __global char *t_interactedTrue, __constant Bubble *t_bubble,
    __constant double *t_dt, __constant double *t_m_in,
   ...
   * If particle.x > R_cyclic 				-> Then particle.x = particle.x - 2
   * * R_cyclic If particle.x < -R_cyclic				-> Then
   * particle.x = particle.x + 2 * R_cyclic
   */
 ...
 * Simulate
 */
__kernel void particles_step_cyclic(__global Particle *t_particles,
                                    __constant double *t_cycleRadius,
...
__kernel void assign_cell_index_to_particle(
    __global Particle *t_particles, __global const unsigned int *maxCellIndex,
    __global const double *cellLength, __global const double *cuboidShift) {
  unsigned int gid = get_global_id(0);

  Particle particle = t_particles[gid];
  // Find cell numbers
  int x_index = (int)((particle.x + cellLength[0] * maxCellIndex[0] / 2 +
                       cuboidShift[0]) /
                      cellLength[0]);
  int y_index = (int)((particle.y + cellLength[0] * maxCellIndex[0] / 2 +
                       cuboidShift[1]) /
                      cellLength[0]);
  int z_index = (int)((particle.z + cellLength[0] * maxCellIndex[0] / 2 +
                       cuboidShift[2]) /
                      cellLength[0]);
  ...
  if ((x_index < 0) || (x_index >= maxCellIndex[0])) {
    ...
    t_particles[gid].idxCollisionCell =
...
__kernel void assign_particle_cell_index_two_phase(
    __global Particle *t_particles, __global const int *maxCellIndex,
  ...
  if ((a < 0) || (a >= maxCellIndex[0])) {
    ...
    t_particles[gid].idxCollisionCell = 1 + a + b * maxCellIndex[0] +
                                        c * maxCellIndex[0] * maxCellIndex[1] +
                                        particle.b_inBubble * maxCellIndex[0] *
                                            maxCellIndex[1] * maxCellIndex[2];
  }
}

__kernel void label_particles_position_by_coordinate(
    __global Particle *t_particles, __global Bubble *t_bubble) {
  unsigned int gid = get_global_id(0);

  // If R_b^2 > R_x^2 then particle is inside the bubble
  t_particles[gid].b_inBubble =
      fma(t_particles[gid].x, t_particles[gid].x,
          fma(t_particles[gid].y, t_particles[gid].y,
              t_particles[gid].z * t_particles[gid].z)) < t_bubble[0].radius2;
}

__kernel void label_particles_position_by_mass(__global Particle *t_particles,
...
__kernel void transform_momentum(__global Particle *t_particles,
  ...
  if (particle.idxCollisionCell != 0) {
    ...
    if ((cell.particle_count > 1) && (cell.mass != 0)) {
      ...
      p0 = cell.gamma * (particle.E - particle.pX * cell.vX -
                         particle.pY * cell.vY - particle.pZ * cell.vZ);
      ...
      p1 = particle.pX * (cos_theta + pow(cell.x, 2) * one_minus_cos_theta) +
           particle.pY *
               (cell.x * cell.y * one_minus_cos_theta - cell.z * sin_theta) +
           particle.pZ *
               (cell.x * cell.z * one_minus_cos_theta + cell.y * sin_theta);
      p2 = particle.pX *
               (cell.x * cell.y * one_minus_cos_theta + cell.z * sin_theta) +
           particle.pY * (cos_theta + pow(cell.y, 2) * one_minus_cos_theta) +
           particle.pZ *
               (cell.y * cell.z * one_minus_cos_theta - cell.x * sin_theta);
      p3 = particle.pX *
               (cell.x * cell.z * one_minus_cos_theta - cell.y * sin_theta) +
           particle.pY *
               (cell.y * cell.z * one_minus_cos_theta + cell.x * sin_theta) +
...
__kernel void particle_bounce(
  ...
  particle.pX = (boundaryDistanceFromCenter[0] > fabs(particle.x)) *
                    particle.pX +  // If inside, leave momentum alone
                ((boundaryDistanceFromCenter[0] < fabs(particle.x)) &&
                 (-boundaryDistanceFromCenter[0] > particle.x)) *
                    fabs(particle.pX) -  // particle too -, chane to +
                ((boundaryDistanceFromCenter[0] < fabs(particle.x)) &&
                 (boundaryDistanceFromCenter[0] < particle.x)) *
                    fabs(particle.pX);  // particle too +, chane to -
  particle.pY =
      (boundaryDistanceFromCenter[0] > fabs(particle.y)) * particle.pY +
      ((boundaryDistanceFromCenter[0] < fabs(particle.y)) &&
       (-boundaryDistanceFromCenter[0] > particle.y)) *
          fabs(particle.pY) -
      ((boundaryDistanceFromCenter[0] < fabs(particle.y)) &&
       (boundaryDistanceFromCenter[0] < particle.y)) *
          fabs(particle.pY);
  particle.pZ =
      (boundaryDistanceFromCenter[0] > fabs(particle.z)) * particle.pZ +
      ((boundaryDistanceFromCenter[0] < fabs(particle.z)) &&
       (-boundaryDistanceFromCenter[0] > particle.z)) *
          fabs(particle.pZ) -
      ((boundaryDistanceFromCenter[0] < fabs(particle.z)) &&
       (boundaryDistanceFromCenter[0] < particle.z)) *
          fabs(particle.pZ);
