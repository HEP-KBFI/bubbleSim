...
OpenCLWrapper::OpenCLWrapper(
    ...
    std::vector<numType>& t_X, std::vector<numType>& t_P,
    std::vector<numType>& t_E, std::vector<numType>& t_M,
    std::vector<numType>& t_dP, numType& t_dt, numType& t_massTrue,
    numType& t_massFalse, numType t_massDelta2, numType& t_bubbleRadius,
...
void OpenCLWrapper::readBufferX(std::vector<numType>& t_dataX) {
  m_queue.enqueueReadBuffer(m_bufferX, CL_TRUE, 0,
                            t_dataX.size() * sizeof(numType), t_dataX.data());
}

void OpenCLWrapper::readBufferP(std::vector<numType>& t_dataP) {
  m_queue.enqueueReadBuffer(m_bufferP, CL_TRUE, 0,
                            t_dataP.size() * sizeof(numType), t_dataP.data());
}
...
void OpenCLWrapper::readBufferM(std::vector<numType>& t_dataM) {
  m_queue.enqueueReadBuffer(m_bufferM, CL_TRUE, 0,
                            t_dataM.size() * sizeof(numType), t_dataM.data());
}

void OpenCLWrapper::readBufferE(std::vector<numType>& t_dataE) {
  m_queue.enqueueReadBuffer(m_bufferE, CL_TRUE, 0,
                            t_dataE.size() * sizeof(numType), t_dataE.data());
}
...
void OpenCLWrapper::readBufferR(numType& t_dataR) {
  m_queue.enqueueReadBuffer(m_bufferBubbleRadius, CL_TRUE, 0, sizeof(numType),
                            &t_dataR);
}

void OpenCLWrapper::readBufferSpeed(numType& t_dataSpeed) {
  m_queue.enqueueReadBuffer(m_bufferBubbleSpeed, CL_TRUE, 0, sizeof(numType),
                            &t_dataSpeed);
}
