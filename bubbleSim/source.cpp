...
void createSimulationInfoFile(std::filesystem::path filePath, int t_seed,
  ...
  simulationListStream << filePath.filename() << "," << t_seed << ","
                       ...
                       << t_countTrue << "," << t_coupling << "," << t_dV << "," << t_n << ","
 ...
 */
  ...
  */
        ...
        if (b_toStream) {
          ...
          if (b_streamProfile) {
            if (b_streamProfile) {
              dataStreamer.streamProfiles(
                  nStream, rhoStream, pStreamIn, pStreamOut, i_densityBins,
                  i_momentumBins, 1.5 * initialBubbleRadius,
                  temperatureFalse * 15, sim.getEnergyDensityFalseSimInitial());
            }
          }
          dataStreamer.reset();
        }
  ...
  createSimulationInfoFile(
      ...
      sim.get_dt(),
      dataStreamer.countMassRadiusDifference(b_isBubbleTrueVacuum), (int)ms_int.count());
  std::cout << std::endl
