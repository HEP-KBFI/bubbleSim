...
DataStreamer::DataStreamer(Simulation& t_sim, Bubble& t_bubble, OpenCLWrapper& t_openCLWrapper) : m_sim(t_sim), m_bubble(t_bubble), m_openCLWrapper(t_openCLWrapper) {
	m_readBufferX = true;
	m_readBufferP = true;
...
void DataStreamer::streamMassRadiusDifference(bool t_isBubbleTrueVacuum) {
	...
	std::printf("\nTotal count (M/R): %.5f\n", (double)(countMassTrue + countMassFalse)/(countRadiusTrue + countRadiusFalse));
	std::printf("True/False vacuum difference (M-R): %d / %d\n", countMassTrue - countRadiusTrue, countMassFalse - countRadiusFalse);
