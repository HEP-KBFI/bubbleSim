...
CollisionCellCollection::CollisionCellCollection(
  ...
  /* TODO
  * Add label to particles which distinguises if particle is inside or outside the bubble -> collision cell count gets 2x bigger
  * 
  */
...
void CollisionCellCollection::recalculate_cells(
  ...
  for (size_t i = 1; i < m_cellCount; i++) {
    ...
    if (frames[i][4] > 1) {
      ...
      // Generate parameters for rotation axis
      ...
      m_collisionCells[i].theta = (numType)2. * (numType)M_PI * t_rng.generate_number();
